...
# -*- coding: utf-8 -*-
"""
Script para mapear bandeiras dos municípios e gerar arquivo JSON
para ser usado no frontend
"""
...
def encontrar_bandeira(municipio, bandeiras_existentes):
    ...
    if municipio in mapeamento_manual:
        arquivo = mapeamento_manual[municipio]
        if arquivo in bandeiras_existentes:
            return arquivo
    ...
    for bandeira in bandeiras_existentes:
        ...
        for variacao in variacoes:
            ...
            if variacao_norm in bandeira_normalizada or bandeira_normalizada in variacao_norm:
                # Verifica se é uma correspondência significativa (mais de 5 caracteres)
                if len(variacao_norm) > 5 and len(bandeira_normalizada) > 5:
                    return bandeira
...
def main():
    """Função principal"""
    print("\n" + "=" * 80)
    print("MAPEAMENTO DE BANDEIRAS DOS MUNICÍPIOS DO CIMCERO")
    print("=" * 80 + "\n")
